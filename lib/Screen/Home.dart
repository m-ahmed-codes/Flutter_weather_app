...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    // print("object");
    context.read<WeatherapiHomeBloc>().add(WeatherApiHomeEvent());
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocBuilder<WeatherapiHomeBloc, WeatherapiState>(
        builder: (context, state) {
          if (state is WeatherapiLoadingState) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          } else if (state is WeatherApiLoadedState) {
            if (state.root.forecast!.forecastday != null) {
              for (var i = 0;
                  i < state.root.forecast!.forecastday!.length;
                  i++) {
                if (state.root.forecast!.forecastday != null) {
                  forecastday.add(state.root.forecast!.forecastday![i]);
                }
              }
            print('state hahah ${state.root.forecast!.forecastday!.length}');
            }
            return Container(
              height: myHeight,
              width: mywidth,
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    "${state.root.location!.name}",
                    style: const TextStyle(fontSize: 30),
                  ),
                  const SizedBox(
                    height: 25,
                  ),
                  Text("${state.root.location!.localtime!.substring(0, 10)}",
                    ...
                    child: Row(
                      children: [
                        Expanded(
                            flex: 2,
                            child: Container(
                              child: const Center(child: Text('Forecast')),
                              decoration: BoxDecoration(
                                  borderRadius: BorderRadius.circular(10),
                  ...
                  Image.network("https:${state.root.current!.condition!.icon}",
                      scale: .3),
                  const SizedBox(
                    height: 21,
                  ),
                      ...
                      children: [
                        Expanded(
                            ...
                            child: Column(
                              children: [
                                const Text("Temp"),
                                Text("${state.root.current!.tempc.toString()}"),
                              ],
                            )),
                        Expanded(
                            flex: 2,
                            child: Column(
                              children: [
                                const Text("Temp"),
                                Text(
                                    "${state.root.current!.windkph.toString()} km/h"),
                              ],
                            )),
                        Expanded(
                            flex: 2,
                            child: Column(
                              children: [
                                const Text("Temp"),
                                Text(
                                    "${state.root.current!.humidity.toString()} km/h"),
                              ],
                            )),
                      ],
                    ),
          ...
          } else if (state is WeatherApiErrorState) {
            return Center(
